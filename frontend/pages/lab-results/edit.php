...
$formData = [
    ...
    'test_type' => $labResult['test_type'] ?? 'HbA1c',
    'hba1c' => $labResult['hba1c'] ?? '',
    'fasting_glucose' => $labResult['fasting_glucose'] ?? '',
    'random_glucose' => $labResult['random_glucose'] ?? '',
    'cholesterol_total' => $labResult['cholesterol_total'] ?? '',
    'cholesterol_ldl' => $labResult['cholesterol_ldl'] ?? '',
    'cholesterol_hdl' => $labResult['cholesterol_hdl'] ?? '',
    'triglycerides' => $labResult['triglycerides'] ?? '',
    'creatinine' => $labResult['creatinine'] ?? '',
    'egfr' => $labResult['egfr'] ?? '',
    'blood_pressure_systolic' => $labResult['blood_pressure_systolic'] ?? '',
    'blood_pressure_diastolic' => $labResult['blood_pressure_diastolic'] ?? '',
    'weight' => $labResult['weight'] ?? '',
    ...
    if (!validateCsrfToken(post('csrf_token'))) {
        ...
        if (empty($errors)) {
            ...
            if ($response['success']) {
                ...
                $errors[] = $response['error']['message'] ?? 'Failed to update lab result.';
            }
        }
    }
}
...
<div style="max-width: 900px; margin: 0 auto;">
    ...
    <div class="page-header">
        <div class="flex items-center gap-4">
            ...
            <div>
                ...
                <p class="page-subtitle"><?php echo formatDate($labResult['test_date'], 'M j, Y'); ?> - <?php echo e($labResult['test_type'] ?? 'Test'); ?></p>
            </div>
        </div>
    </div>
    ...
    <form method="POST" action="<?php echo baseUrl('/lab-results/edit?id=' . $labResultId); ?>">
                ...
                <div class="form-grid">
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" name="blood_pressure_systolic" class="form-input" min="0" max="300"
                               value="<?php echo e($formData['blood_pressure_systolic']); ?>">
                    </div>
                    <div class="form-group">
                        ...
                        <input type="number" name="blood_pressure_diastolic" class="form-input" min="0" max="200"
                               value="<?php echo e($formData['blood_pressure_diastolic']); ?>">
                    </div>
                    <div class="form-group">
                        <label class="form-label">Weight (kg)</label>
                        <input type="number" name="weight" class="form-input" step="0.1" min="0" max="500"
                               value="<?php echo e($formData['weight']); ?>">
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="card">
            ...
            <div class="card-body">
                <div class="form-group">
                    <textarea name="notes" class="form-input" rows="3"><?php echo e($formData['notes']); ?></textarea>
                </div>
            </div>
        </div>
